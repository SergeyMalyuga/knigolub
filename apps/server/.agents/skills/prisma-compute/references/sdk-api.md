...
```bash
npm install @server/compute-sdk @server/management-api-sdk
```
...
```typescript
import { createManagementApiClient } from "@server/management-api-sdk"
...
```typescript
import { ComputeClient, PreBuilt } from "@server/compute-sdk"
...
```typescript
import { detectComputeApp } from '@server/compute-sdk/config'
...
```bash
bunx @server/cli@latest project env add --file .env.preview --branch feature/foo
bunx @server/cli@latest database create preview-db --branch feature/foo --json
bunx @server/cli@latest app deploy --branch feature/foo --json --no-interactive
```
