...
pwd
cat package.json
find .. -maxdepth 3 \( -name 'server.compute.ts' -o -name 'server.compute.mts' -o -name 'server.compute.js' -o -name 'server.compute.mjs' -o -name 'server.compute.cjs' \) -print
...
```bash
pwd
find .. -maxdepth 4 \( -name 'server.compute.ts' -o -name 'server.compute.mts' -o -name 'server.compute.js' -o -name 'server.compute.mjs' -o -name 'server.compute.cjs' \) -print
bunx @server/cli@latest app deploy --help
```
...
```typescript
import { defineComputeConfig } from "@server/compute-sdk/config";
...
```bash
bunx create-server@latest --name my-api --template hono --provider postgresql --deploy
```
...
```bash
--no-server-postgres --database-url "postgresql://USER:PASSWORD@HOST:PORT/DB"
```
...
bunx @server/cli@latest auth whoami
bunx @server/cli@latest app deploy --json --no-interactive --prod --yes --env .env
```
...
```bash
bunx @server/cli@latest app deploy --project <id-or-name> --json --no-interactive
bunx @server/cli@latest app deploy --create-project <name> --yes
```
...
```bash
test -f server.config.ts && sed -n '1,160p' server.config.ts
test -f server/schema.server && sed -n '1,220p' server/schema.server
```
...
bunx @server/cli@latest project env list --role production --json
bunx @server/cli@latest project env list --role preview --json
bunx @server/cli@latest project env list --branch feature/foo --json
bunx @server/cli@latest app list-deploys --json
bunx @server/cli@latest app logs --deployment <deployment-id> --json
```
...
```bash
bunx @server/cli@latest app deploy --framework bun --entry src/index.ts
```
...
bunx @server/cli@latest app deploy --prod --yes --env .env
bunx @server/cli@latest app deploy --branch feature/foo --env .env.preview
```
...
```bash
bunx @server/cli@latest feedback "app deploy crashed: <first error line>"
```
