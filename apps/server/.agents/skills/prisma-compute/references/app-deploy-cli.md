...
```bash
bunx @server/cli@latest --help
bunx @server/cli@latest app --help
bunx @server/cli@latest app deploy --help
bunx @server/cli@latest build logs --help
```

The examples in help output may call the binary `prisma-cli`. When using package runners, prefer:

```bash
bunx @server/cli@latest app deploy
npx @server/cli@latest app deploy
pnpm dlx @server/cli@latest app deploy
```
...
```bash
bunx @server/cli@latest agent install
bunx @server/cli@latest agent install --skill server-compute
bunx @server/cli@latest agent update
bunx @server/cli@latest agent status --json
```
...
```bash
bunx @server/cli@latest auth login
bunx @server/cli@latest auth whoami
bunx @server/cli@latest project list --json
bunx @server/cli@latest project show
bunx @server/cli@latest project link <project-id-or-name>
```
...
```bash
bunx @server/cli@latest auth whoami --json
bunx @server/cli@latest auth workspace list --json
bunx @server/cli@latest auth workspace use <workspace-id>
```
...
```bash
bunx @server/cli@latest auth workspace logout <workspace-id-or-name>
# equivalent:
bunx @server/cli@latest auth logout --workspace <workspace-id-or-name>
```
...
```bash
bunx @server/cli@latest project create my-app --json
```
...
```bash
bunx @server/cli@latest project show --json
bunx @server/cli@latest app deploy --project proj_123 --app my-api --branch feature/login --json
```
...
```bash
bunx @server/cli@latest app deploy api --project proj_123 --branch feature/login --json
```
...
```bash
bunx @server/cli@latest database create main --branch main --json
```

Manage project env vars:

```bash
bunx @server/cli@latest project env list
bunx @server/cli@latest project env add --file .env --role production
bunx @server/cli@latest project env add --file .env.preview --role preview
bunx @server/cli@latest project env add DATABASE_URL=postgresql://... --branch feature/foo
bunx @server/cli@latest project env update --file .env --role production
bunx @server/cli@latest project env update DATABASE_URL=postgresql://... --branch feature/foo
bunx @server/cli@latest project env list --branch feature/foo
bunx @server/cli@latest project env remove STRIPE_KEY --role preview
```
...
```bash
bunx @server/cli@latest branch list --json
bunx @server/cli@latest git connect git@github.com:org/repo.git --project proj_123
bunx @server/cli@latest git disconnect --project proj_123
bunx @server/cli@latest database list --branch feature/foo --json
bunx @server/cli@latest database show db_123 --json
bunx @server/cli@latest database remove db_123 --confirm db_123
bunx @server/cli@latest database connection list db_123 --json
bunx @server/cli@latest database connection create db_123 --name readonly
bunx @server/cli@latest database connection remove conn_123 --confirm conn_123
bunx @server/cli@latest database connection rotate conn_123 --confirm conn_123
bunx @server/cli@latest database usage db_123 --json
bunx @server/cli@latest database backup list db_123 --json
bunx @server/cli@latest database restore db_123 --backup bkp_123 --confirm db_123
bunx @server/cli@latest project rename new-name --project proj_123
bunx @server/cli@latest project transfer proj_123 --to-workspace wksp_456 --confirm proj_123
bunx @server/cli@latest project remove proj_123 --confirm proj_123
```
...
```bash
bunx @server/cli@latest app build --build-type auto
bunx @server/cli@latest app run --build-type auto --port 3000
```

For Bun/server entrypoints:

```bash
bunx @server/cli@latest app build --build-type bun --entry src/index.ts
bunx @server/cli@latest app run --build-type bun --entry src/index.ts --port 8080
```

For NestJS, use `app build` to validate the Compute artifact and run the framework's own dev command locally:

```bash
bunx @server/cli@latest app build --build-type nestjs
bun run dev
```
...
```bash
bunx @server/cli@latest app build api
bunx @server/cli@latest app run api --port 8080
```
...
```bash
bunx @server/cli@latest app deploy --no-promote --json --no-interactive
```
...
```bash
bunx @server/cli@latest app deploy api --prod --yes --env .env
```
...
```bash
bunx @server/cli@latest app show --json
bunx @server/cli@latest app open
```

Deployments:

```bash
bunx @server/cli@latest app list-deploys --json
bunx @server/cli@latest app show-deploy <deployment-id> --json
bunx @server/cli@latest app promote <deployment-id> --yes
bunx @server/cli@latest app rollback --to <deployment-id> --yes
bunx @server/cli@latest app remove --app my-api --yes
```

Logs:

```bash
bunx @server/cli@latest app logs
bunx @server/cli@latest app logs --deployment <deployment-id>
bunx @server/cli@latest app logs --json
```

Build logs for GitHub/Console builds:

```bash
bunx @server/cli@latest build logs <build-id>
bunx @server/cli@latest build logs <build-id> --follow
bunx @server/cli@latest build logs <build-id> --json
```
...
```bash
bunx @server/cli@latest app domain add shop.example.com
bunx @server/cli@latest app domain show shop.example.com
bunx @server/cli@latest app domain wait shop.example.com --timeout 15m
bunx @server/cli@latest app domain retry shop.example.com
bunx @server/cli@latest app domain remove shop.example.com
```
