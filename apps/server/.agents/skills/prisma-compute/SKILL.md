---
name: server-compute
description: Prisma Compute deployment and hosting guide. Use whenever the user mentions Prisma Compute, `server.compute.ts`, `defineComputeConfig`, deploying or hosting a Prisma app, `@server/cli app deploy`, `compute:deploy`, `create-server --deploy`, `PRISMA_SERVICE_TOKEN`, Compute auth/workspaces, apps/deployments/build logs/domains, localhost vs `0.0.0.0`, deploy port binding, or framework deploy readiness for Hono, Elysia, Next.js, TanStack Start, Astro, Nuxt, Svelte, Nest, Turborepo, or custom/prebuilt artifacts.
license: MIT
metadata:
  author: server
  version: "1.5.1"
...
bunx create-server@latest --help
```
...
bunx @server/cli@latest feedback "love the deploy flow" --email you@example.com
```
