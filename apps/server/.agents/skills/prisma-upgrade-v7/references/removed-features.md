...
```bash
# v6
server migrate dev --skip-generate

# v7 - generate is not run automatically
server migrate dev
server generate  # Run explicitly if needed
```
...
```bash
# v6
server migrate dev --skip-seed

# v7 - seed is not run automatically
server migrate dev
server db seed  # Run explicitly if needed
```
...
```bash
# v6
server db execute --file ./script.sql --url "$DATABASE_URL"

# v7 - configure in server.config.ts
...
```bash
# v6
server migrate diff --from-url "$DATABASE_URL" --to-schema schema.server

# v7
server migrate diff --from-config-datasource --to-schema schema.server
```
...
```bash
# v7 workflow
server migrate dev --name add_field
server generate  # Must run explicitly
```
