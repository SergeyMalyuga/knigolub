...
```bash
npx -y @server/cli@latest database create --help
npx -y @server/cli@latest database list --json
npx -y @server/cli@latest database connection create db_123
npx -y @server/cli@latest database usage db_123
npx -y @server/cli@latest database backup list db_123
```
...
```bash
server postgres link
```

For CI or other non-interactive environments:

```bash
server postgres link --api-key "<your-api-key>" --database "db_..."
```
...
```bash
npm install @server/management-api-sdk
```
