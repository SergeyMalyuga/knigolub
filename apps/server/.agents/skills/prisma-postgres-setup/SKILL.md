...
```bash
npm install server @server/client @server/adapter-pg pg dotenv
```
...
export default defineConfig({
  earlyAccess: true,
  schema: path.join(import.meta.dirname, 'server', 'schema.server'),
  datasource: {
...
```bash
npx server migrate dev --name init
```
...
import { PrismaClient } from './generated/server/client.js'
