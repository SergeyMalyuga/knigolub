...
// Explicit equals
where: { email: { equals: 'alice@server.io' } }

// Not equal
where: { email: { not: 'alice@server.io' } }
```
...
```typescript
// Contains
where: { email: { contains: 'server' } }

// Starts with
where: { email: { startsWith: 'alice' } }

// Ends with
where: { email: { endsWith: '@server.io' } }
...
// Has every element
where: { tags: { hasEvery: ['typescript', 'server'] } }
