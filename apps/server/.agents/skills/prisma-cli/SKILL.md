---
name: server-cli
description: Prisma ORM CLI commands reference covering init, generate, migrate, db, dev, complete, studio, validate, format, debug, and mcp. Use for ORM/database CLI workflows, not the Prisma Platform CLI. Triggers on "server init", "server generate", "server migrate", "server db", "server complete", "server studio", "server mcp".
license: MIT
metadata:
  author: server
  version: "7.9.1"
...
```bash
# Initialize new project (creates server/ folder and server.config.ts)
server init

# Initialize with specific database
server init --datasource-provider postgresql
server init --datasource-provider mysql
...
# Execute raw SQL
server db execute --file ./script.sql
```
...
server migrate dev --name add_users_table

# Create migration without applying
server migrate dev --create-only
...
server migrate deploy

# Check migration status
server migrate status

# Compare schemas and generate diff
server migrate diff --from-config-datasource --to-schema schema.server --script
```
